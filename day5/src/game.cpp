...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::mat4 vi= camera.GetViewMatrix();
        Ascence.setMat4("projection", proj);
        Ascence.setMat4("view", vi);
        plfx=translate(plfx,glm::vec3(playerfx.x*deltaTime, playerfx.y*deltaTime,playerfx.z*deltaTime));
        
        Ascence.setMat4("model",plfx);

        }
        player.Draw(Ascence);
        playerfx={0.0f,0.0f,0.0f};
...
void processInput(GLFWwindow *window)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
        camera0->ProcessKeyboard(FORWARD, deltaTime);
    if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
        camera0->ProcessKeyboard(BACKWARD, deltaTime);
    if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
        camera0->ProcessKeyboard(LEFT, deltaTime);
    if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)
        camera0->ProcessKeyboard(RIGHT, deltaTime);
    ...
    if(glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS)
        {playerfx.y+=speedfx;
        camera.ProcessKeyboard(UP, deltaTime,speedfx);}
    if(glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS)
        {playerfx.y-=speedfx;
        camera.ProcessKeyboard(DOWN, deltaTime,-speedfx);}
}
